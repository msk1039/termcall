...
func renderControls(termW int, camOn, micOn, showStats bool, renderMode string, theme Theme) string {
	...
	// Right segments: [N] Mode -> [S] Stats -> [Q] Quit
	...
	r3Sep := lipgloss.NewStyle().Background(sBg).Foreground(qBg).Render("")
	r3 := lipgloss.NewStyle().Background(qBg).Foreground(qFg).Bold(true).Render(qText)

	rightSection := lipgloss.JoinHorizontal(lipgloss.Top, r1Sep, r1, r2Sep, r2, r3Sep, r3)
